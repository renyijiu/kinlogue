...
只有逐字节确认相同且比较完成后旧候选仍在有效选择列表中的文件才在手机侧忽略。浏览器不支持读取、读取失败、候选过多、超时或超过字节预算时，都会回退到正常上传，由 Mac 做权威去重；不能因为元数据相似而丢弃文件。长比较会主动让出浏览器事件循环，用户删除项目会取消对应比较；比较期间移除旧候选不会丢弃新选择。
...
会话文件项的 1,000 项限额同时计算已登记项和正在等待存储读取的预留项；并发预留不会超限，也不会使后续会话状态响应无法编码。已登记文件的重试继续复用原文件项。
