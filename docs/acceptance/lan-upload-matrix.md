...
| Mac / 客户端 | 必测场景 | 状态 |
| --- | --- | --- |
| macOS 26 + 指定版本 iOS Safari | 地址/二维码、配对、重复追加、多文件独立状态、本地重复选择、重试、Mac 队列合并/多选归档、停止与锁屏生命周期 | 未验证；正式发布前需要真实设备 |
| macOS 26 + 指定版本 Android Chrome | 同上 | 未验证；正式发布前需要真实设备 |
| macOS 27 + 指定版本 iOS Safari | 同上 | 未验证；正式发布前需要真实设备 |
| macOS 27 + 指定版本 Android Chrome | 同上 | 未验证；正式发布前需要真实设备 |
