...
| 手机投递 | 只在用户主动开启的临时会话中工作，使用普通 HTTP；仅适合可信任的私人 Wi-Fi 或有线局域网。 |
...
项目尚未公开发布。当前版本、最低系统、自动化状态和人工门禁只在 [当前候选证据](docs/acceptance/current-release.md) 维护；验证命令见 [测试与发布](docs/testing-and-release.md)。
