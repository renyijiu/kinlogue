...
struct DICOMPackagingBoundaryTests {
    ...
    func generatedFixtureProbeExercisesTheStrictlySignedEmbeddedXPC() throws {
        ...
        #expect(helper.contains("mkstemp"))
        let unlink = try #require(helper.range(of: "Darwin.unlink"))
        let copy = try #require(helper.range(of: "output.write(contentsOf: chunk)"))
        #expect(unlink.upperBound < copy.lowerBound)
        #expect(!helper.contains("createFile"))
        #expect(!helper.contains("createDirectory"))
        #expect(helper.contains("metadata.st_size == request.declaredByteCount"))
